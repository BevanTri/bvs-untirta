    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="card p-5 border-l-4 border-brand-gold">
                ...
                <p class="font-display text-lg sm:text-2xl font-bold text-brand-ink mt-0.5 font-mono tabular-nums">Rp{{ number_format($totalRevenue,0,',','.') }}</p>
            </div>
            <div class="card p-5 border-l-4 border-brand-blue">
                ...
                <p class="font-display text-lg sm:text-2xl font-bold text-brand-blue mt-0.5 font-mono tabular-nums">{{ $orderCount }} <span class="text-xs font-normal text-brand-ink-muted">Rp{{ number_format($orderRevenue,0,',','.') }}</span></p>
            </div>
            <div class="card p-5 border-l-4 border-sky-500">
                ...
                <p class="font-display text-lg sm:text-2xl font-bold text-sky-600 mt-0.5 font-mono tabular-nums">{{ $repairCount }} <span class="text-xs font-normal text-brand-ink-muted">Rp{{ number_format($repairRevenue,0,',','.') }}</span></p>
            </div>
            <div class="card p-5 border-l-4 border-emerald-600">
                ...
                <p class="font-display text-lg sm:text-2xl font-bold text-emerald-700 mt-0.5 font-mono tabular-nums">{{ $orderCount + $repairCount }}</p>
            </div>
        </div>

        <div class="flex flex-col gap-6">
            <div class="card p-0 overflow-hidden">
                <div class="flex items-center justify-between px-5 py-4 border-b border-brand-border bg-brand-warm/30">
                    <h3 class="font-display font-bold text-brand-ink text-sm tracking-wide">Pesanan Produk</h3>
                ...
                <div class="table-wrap overflow-x-auto">
                    <table class="table-base">
                        <thead>
                            <tr class="border-b bg-brand-warm/70 text-brand-ink-faint text-xs uppercase tracking-widest">
                                <th class="px-5 py-3 text-left font-semibold">Invoice</th>
                                <th class="px-5 py-3 text-left font-semibold hidden sm:table-cell">Customer</th>
                                <th class="px-5 py-3 text-right font-semibold">Total</th>
                                <th class="px-5 py-3 text-center font-semibold">Status</th>
                                <th class="px-5 py-3 text-center font-semibold hidden md:table-cell">Tanggal</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($orders as $o)
                            <tr class="border-b border-brand-border/30 hover:bg-brand-warm/40 transition-colors">
                                <td class="px-5 py-3 font-mono text-xs whitespace-nowrap">{{ $o->order_number }}</td>
                                <td class="px-5 py-3 hidden sm:table-cell whitespace-nowrap">{{ $o->user->name ?? '-' }}</td>
                                ...
                                <td class="px-5 py-3 text-center text-brand-ink-muted text-xs hidden md:table-cell whitespace-nowrap">{{ $o->created_at->format('d/m/Y H:i') }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
            ...
            <div class="card p-0 overflow-hidden">
                <div class="flex items-center justify-between px-5 py-4 border-b border-brand-border bg-brand-warm/30">
                    <h3 class="font-display font-bold text-brand-ink text-sm tracking-wide">Servis Workshop</h3>
                    ...
                    <table class="table-base">
                        <thead>
                            <tr class="border-b bg-brand-warm/70 text-brand-ink-faint text-xs uppercase tracking-widest">
                                ...
                                <th class="px-5 py-3 text-left font-semibold hidden sm:table-cell">Kendaraan</th>
                                <th class="px-5 py-3 text-right font-semibold">Total</th>
                                <th class="px-5 py-3 text-center font-semibold">Status</th>
                                <th class="px-5 py-3 text-center font-semibold hidden md:table-cell">Tanggal</th>
                            </tr>
                        ...
                        <tbody>
                            ...
                            <tr class="border-b border-brand-border/30 hover:bg-brand-warm/40 transition-colors">
                                <td class="px-5 py-3 font-mono text-xs whitespace-nowrap">{{ $r->order_number }}</td>
                                <td class="px-5 py-3 whitespace-nowrap">{{ $r->customer->name }}</td>
                                <td class="px-5 py-3 hidden sm:table-cell text-brand-ink-muted text-xs whitespace-nowrap">{{ $r->vehicle->plate_number }} ({{ $r->vehicle->brand }})</td>
                                ...
                                <td class="px-5 py-3 text-center text-brand-ink-muted text-xs hidden md:table-cell whitespace-nowrap">{{ $r->created_at->format('d/m/Y') }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
