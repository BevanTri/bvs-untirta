...
<div class="max-w-4xl mx-auto px-4 py-6 md:py-10">
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="font-display text-2xl md:text-3xl font-bold text-brand-ink dark:text-zinc-100 uppercase tracking-wide">Keranjang</h1>
            @if($items->isNotEmpty())
            <p class="text-sm text-brand-ink-muted dark:text-zinc-400">{{ $items->count() }} item</p>
            @endif
        </div>
    </div>
            ...
            <div class="card p-4 flex items-start gap-4 active:scale-[0.99] transition-transform">
                <div class="shrink-0">
                    <label class="flex items-center justify-center w-6 h-6 cursor-pointer">
                        <input type="checkbox" name="selected[]" value="{{ $item->id }}" class="w-4 h-4 rounded border-brand-border dark:border-brand-navy-3 text-brand-gold focus:ring-brand-gold/30" checked>
                    </label>
                </div>
                <div class="w-16 h-16 shrink-0 rounded-xl bg-brand-warm dark:bg-brand-navy-3/30 flex items-center justify-center overflow-hidden">
                    ...
                    @else
                    <svg class="w-8 h-8 text-brand-ink-faint dark:text-zinc-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>
                    @endif
                </div>
                <div class="flex-1 min-w-0">
                    <h3 class="text-sm font-medium text-brand-ink dark:text-zinc-100 line-clamp-1">{{ $item->name }}</h3>
                    <p class="text-xs text-brand-ink-muted dark:text-zinc-400 mt-0.5 font-mono">Rp{{ number_format($item->unit_price, 0, ',', '.') }}</p>
                    <div class="flex items-center gap-3 mt-2">
                        <label class="text-xs text-brand-ink-faint dark:text-zinc-500">Qty:</label>
        ...
        <div class="sticky bottom-16 md:bottom-0 z-30 -mx-4 px-4 pb-4 md:pb-0 md:static md:mx-0 md:px-0">
            <div class="card p-4 md:p-5 shadow-level-4 md:shadow-level-1">
                <div class="flex items-center justify-between mb-3">
                    <span class="text-sm text-brand-ink-muted dark:text-zinc-400">Subtotal</span>
