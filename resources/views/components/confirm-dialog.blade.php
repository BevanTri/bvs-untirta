...
<div id="{{ $name }}" class="fixed inset-0 z-[80] items-center justify-center bg-black/50 hidden" style="display:none" onclick="if(event.target===this){this.style.display='none';this.classList.remove('flex')}">
    <div class="bg-white dark:bg-brand-navy-2 rounded-2xl shadow-2xl max-w-sm w-full mx-4 p-6" onclick="event.stopPropagation()">
        <h3 class="font-display font-bold text-lg text-brand-ink dark:text-zinc-100 mb-2">{{ $title }}</h3>
        <p class="text-sm text-brand-ink-muted dark:text-zinc-400 mb-6">{{ $message }}</p>
