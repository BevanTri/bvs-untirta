<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="font-display font-bold text-xl text-brand-ink dark:text-zinc-100 uppercase tracking-wide">Pesanan Saya</h2>
                <p class="text-xs text-brand-ink-muted dark:text-zinc-400">Riwayat pesanan dan servis</p>
            </div>
        </div>
    </x-slot>
            ...
            <a href="{{ $isRepair ? route('repairs.show', $order) : route('orders.show', $order) }}" class="card-hover p-5 mb-4 flex items-center justify-between gap-4 group active:scale-[0.99] transition-all duration-200">
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 flex-wrap">
                        <p class="font-semibold text-brand-ink dark:text-zinc-100 group-hover:text-brand-blue transition-colors">{{ $order->order_number }}</p>
                        <span class="badge {{ $isRepair ? 'badge-warning' : 'badge-info' }} text-[10px] px-2 py-0.5">{{ $isRepair ? 'Servis' : 'Produk' }}</span>
                    </div>
                    <p class="text-sm text-brand-ink-muted dark:text-zinc-400 mt-0.5">{{ $order->created_at->format('d M Y H:i') }}</p>
                    <div class="flex items-center gap-2 mt-2 flex-wrap">
                        ...
                        @endif
                        <span class="text-xs text-brand-ink-faint dark:text-zinc-500">Status: <span class="font-semibold text-brand-ink dark:text-zinc-100">{{ ucfirst($order->status) }}</span></span>
                    </div>
                </div>
                <div class="text-right shrink-0">
                    <p class="font-bold text-lg text-brand-gold font-mono tabular-nums">Rp{{ number_format($order->total, 0, ',', '.') }}</p>
                    <p class="text-xs text-brand-blue dark:text-brand-blue-light group-hover:underline mt-1">Detail &rarr;</p>
                </div>
            </a>
            @empty
            <x-empty-state
