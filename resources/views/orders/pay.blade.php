        ...
        <div class="max-w-2xl mx-auto px-4 space-y-6">
            ...
            @php
                ...
                $steps = [
                    ...
                    ['label' => 'Diproses', 'done' => $isProcessing || $isCompleted],
                    ['label' => 'Selesai', 'done' => $isCompleted && $isPaid],
                ];
                $progressWidth = $isCompleted && $isPaid ? '100%' : ($isProcessing || ($isCompleted && !$isPaid) ? '66%' : ($steps[1]['done'] ? '66%' : '33%'));
            @endphp
            ...
            <div class="card p-5">
                ...
                <div class="mt-4 pt-4 border-t border-brand-border dark:border-brand-navy-3 space-y-2">
                    <div class="flex items-center justify-between text-sm">
                        <span class="text-brand-ink-muted dark:text-zinc-400">Subtotal</span>
            ...
            {{-- Payment Form --}}
            <form action="{{ route('payment.checkout', $order) }}" method="POST" class="card p-5">
                @csrf
