...
{{-- Stats --}}
<div class="bg-white dark:bg-brand-navy-2 border-b border-brand-border dark:border-brand-navy-3">
    <div class="max-w-7xl mx-auto px-4 py-8 md:py-10">
        <div class="grid grid-cols-2 gap-6 md:gap-8 max-w-lg mx-auto">
            ...
            <div class="text-center">
                ...
                <div class="text-brand-ink-faint dark:text-zinc-500 text-xs uppercase tracking-[0.15em] mt-2 font-display">Brand Partner</div>
            </div>
        </div>
    </div>
</div>
...
<div class="max-w-7xl mx-auto px-4 py-12 md:py-16">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <a href="{{ route('products.category', $cat) }}" class="card-hover p-5 md:p-6 text-center group border-l-4 border-transparent hover:border-brand-gold active:scale-[0.98] transition-all duration-200">
            ...
            <div class="font-display font-semibold text-base md:text-lg text-brand-ink dark:text-zinc-200 uppercase tracking-wide">{{ $cat->name }}</div>
        </a>
        @endforeach
    </div>
</div>
...
{{-- Products --}}
<div class="bg-brand-warm dark:bg-brand-navy-3/20 border-y border-brand-border dark:border-brand-navy-3 py-12 md:py-16">
    <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            @php
                ...
                if ($hasRealImage) {
                    $imgHtml = '<img src="'.url('uploads/'.$product->image).'" alt="'.$product->name.'" class="w-full h-full object-contain" loading="lazy" onerror="this.onerror=null;this.parentElement.innerHTML=\'<div class=\\\'w-full h-full flex flex-col items-center justify-center bg-brand-warm dark:bg-brand-navy-3/30 text-brand-ink-faint dark:text-zinc-500\\\'><svg class=\\\'w-8 h-8 mb-1\\\' fill=\\\'none\\\' stroke=\\\'currentColor\\\' viewBox=\\\'0 0 24 24\\\'><path stroke-linecap=\\\'round\\\' stroke-linejoin=\\\'round\\\' stroke-width=\\\'1.5\\\' d=\\\'M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z\\\'/><\\\/svg><span class=\\\'text-xs font-medium\\\'>Foto tidak tersedia<\\\/span><\\\/div>\'">';
                } else {
            ...
            <div class="card-hover group active:scale-[0.98] transition-all duration-200">
                ...
                <div class="p-4">
                    ...
                    @else
                    <a href="{{ route('login') }}" class="mt-3 block text-center btn-xs text-brand-ink-muted dark:text-zinc-400 border-2 border-brand-border dark:border-brand-navy-3 py-2 uppercase tracking-wider font-display rounded-lg hover:border-brand-gold hover:text-brand-gold transition-colors">Login untuk Membeli</a>
                    @endauth
                </div>
            </div>
            @endforeach
...
<div class="max-w-7xl mx-auto px-4 py-12 md:py-16">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="card-hover p-6 border-l-4 border-transparent hover:border-brand-gold active:scale-[0.98] transition-all duration-200">
            <h3 class="font-display font-semibold text-xl text-brand-ink dark:text-zinc-200 uppercase">{{ $service->name }}</h3>
            <p class="text-sm text-brand-ink-muted dark:text-zinc-400 mt-2 leading-relaxed">{{ $service->description }}</p>
            <div class="flex items-center justify-between mt-4 pt-4 border-t border-brand-border dark:border-brand-navy-3">
                ...
                @else
                <a href="{{ route('login') }}" class="text-sm font-semibold text-brand-ink-muted dark:text-zinc-400 border-2 border-brand-border dark:border-brand-navy-3 px-4 py-2 rounded-xl hover:border-brand-gold hover:text-brand-gold transition-colors">Login</a>
                @endauth
            </div>
        </div>
        @endforeach
...
<div class="bg-brand-warm dark:bg-brand-navy-3/20 border-y border-brand-border dark:border-brand-navy-3 py-12 md:py-16">
    <div class="max-w-7xl mx-auto px-4">
        <div class="text-center mb-8">
            ...
            <h2 class="font-display text-3xl md:text-4xl font-bold tracking-wide text-brand-ink dark:text-zinc-100">Temukan Kami</h2>
            <p class="text-brand-ink-muted dark:text-zinc-400 text-sm mt-2">Kampus FKIP Untirta, Ciwaru — Cipocok Jaya, Kota Serang</p>
        </div>
        <div class="flex justify-center">
            <div class="rounded-2xl overflow-hidden shadow-level-3 border border-brand-border dark:border-brand-navy-3 w-full max-w-2xl">
...
<div class="max-w-7xl mx-auto px-4 py-12 md:py-16">
    <div class="flex items-end justify-between mb-8">
        ...
        <div class="hidden md:flex items-center gap-2">
            <button id="brand-scroll-prev" class="btn-icon border-2 border-brand-border dark:border-brand-navy-3 text-brand-ink-muted dark:text-zinc-400 hover:border-brand-gold hover:text-brand-gold rounded-xl" aria-label="Scroll kiri">&lsaquo;</button>
            <button id="brand-scroll-next" class="btn-icon border-2 border-brand-border dark:border-brand-navy-3 text-brand-ink-muted dark:text-zinc-400 hover:border-brand-gold hover:text-brand-gold rounded-xl" aria-label="Scroll kanan">&rsaquo;</button>
        </div>
    </div>
    <div class="-mx-4 px-4 overflow-hidden">
        <div id="brand-track" class="flex gap-4 items-center overflow-x-auto scrollbar-hide scroll-smooth pb-2" style="-webkit-overflow-scrolling: touch;">
            ...
            <x-card padding="p-4" class="flex items-center justify-center h-20 w-40 shrink-0 hover:border-brand-gold-light">
                ...
                @else
                <span class="text-sm font-semibold text-brand-ink-muted dark:text-zinc-400 font-display uppercase tracking-wide">{{ $brand->name }}</span>
                @endif
            </x-card>
            @endforeach
        </div>
